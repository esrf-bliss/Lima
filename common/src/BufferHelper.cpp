...
class _BufferHelper_PoolImpl : public BufferHelper::_Impl
{
 ...
 public:
	...
	void prepareBuffers(int nb_buffers, int size) override
	{
		...
		int max_nb_buffers = GetDefMaxNbBuffers(FrameDim(Size(size, 1),
								 Bpp8));
		float factor = m_parameters.reqMemSizePercent * 1e-2;
		int real_nb_buffers = std::max(1, int(max_nb_buffers * factor));
		if ((m_parameters.sizePolicy == Parameters::Automatic) &&
		    (nb_buffers > 0))
			real_nb_buffers = std::min(nb_buffers, real_nb_buffers);
		DEB_TRACE() << DEB_VAR2(real_nb_buffers, size);
		m_pool.allocBuffers(real_nb_buffers, size);
	}
