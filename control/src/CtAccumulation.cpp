...
inline
void CtAccumulation::_SortedDataQueue::prepareInsert()
{
  DEB_MEMBER_FUNCT();
...
void CtAccumulation::readSaturatedImageCounter(Data &saturatedImage,long frameNumber)
{
  ...
  {
    AutoMutex aLock(m_cond.mutex());
    std::deque<Data>& saturated_images = m_saturated_images.getSequentialQueue();
    if(frameNumber < 0)
    {
                                       ...
                                       AutoMutex &aLock)
{
  ...
  if(info.acc_frames == 0) // new Data has to be created
  {
    ...
    m_datas.prepareInsert();
    if(do_sat)
      m_saturated_images.prepareInsert();
...
void CtAccumulation::getFrame(Data &aReturnData,int frameNumber)
{
  ...
  DEB_PARAM() << DEB_VAR1(frameNumber);
  AutoMutex aLock(m_cond.mutex());
  std::deque<Data>& datas = m_datas.getSequentialQueue();
  if(datas.empty())
